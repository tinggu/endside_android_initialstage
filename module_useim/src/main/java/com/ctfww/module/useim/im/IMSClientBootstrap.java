...
//    }
    ...
    public synchronized void init(String userId, String token, String hosts, int appStatus, Context context, IReceiveDataCallback callback) {
        if (!isActive()) {
            ...
            imsClient.init(serverUrlList, new IMSEventListener(userId, token, context, callback), new IMSConnectStatusListener());
        }
    }
    ...
    private Vector<String> convertHosts(String hosts) {
        if (hosts != null && hosts.length() > 0) {
            Log.i("aaaa", "convertHosts: hosts = " + hosts);
            JSONArray hostArray = JSONArray.parseArray(hosts);
            Log.i("bbbbbbbbb", "convertHosts: hosts = " + hosts);
